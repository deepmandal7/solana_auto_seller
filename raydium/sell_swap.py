...
MICROLAMPORTS_PER_SOL = LAMPORTS_PER_SOL * 1000000

def sell(solana_client, TOKEN_TO_SWAP_SELL, payer, token_symbol, S0l_Symbol, transfer_fee, slippage):
    ...
    while txnBool:
        ...
        logger.info("3. Get token Balance from wallet...")
        ...
        while balanceBool:
            ...
            for account in accounts:
                ...
                if mint_in_acc == str(mint):
                    ...
                    lamports_amm = int(account.account.data.parsed['info']['tokenAmount']['amount']) * LAMPORTS_PER_SOL
        ...
        if swap_token_account == None:
            ...
            if WSOL_token_account_Instructions != None:
                swap_tx.add(set_compute_unit_price(transfer_fee=transfer_fee * MICROLAMPORTS_PER_SOL))
                swap_tx.add(
                    transfer(
                        TransferParams(
