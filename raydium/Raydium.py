...
def raydium_swap(ctx, payer, desired_token_address, transfer_fee, slippage):
    ...
    txS = sell(solana_client=ctx, TOKEN_TO_SWAP_SELL=desired_token_address, payer=payer, token_symbol=token_symbol, S0l_Symbol=SOl_Symbol, transfer_fee=transfer_fee, slippage=slippage)
    end_time = time.time()
